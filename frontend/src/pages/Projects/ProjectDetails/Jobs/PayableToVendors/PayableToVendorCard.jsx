...
const FinancePage = () => {
  ...
  const goTo = (path) => { setOpen(false); navigate(`/project/${projectId}/${path}`); };
    ...
    <div className="space-y-8">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {open && (
                <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-lg shadow-lg z-50 overflow-hidden">
                  <button onClick={() => goTo("new-flat-rate-receivable")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">Flat Rate Receivables</button>
                  <button onClick={() => goTo("new-unit-based-receivable")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">Unit Based Receivables</button>
                  <button onClick={() => goTo("new-cat-log")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">CAT Log</button>
                </div>
              )}
            </div>
            <button className="bg-gray-600 text-white text-sm px-3 py-2 rounded shadow hover:bg-gray-700">Export to Excel</button>
          </div>
        </div>

        {receivablesLoading
          ? <div className="text-sm text-gray-400 py-6 text-center">Loading receivables...</div>
          : <ReceivablesTable data={receivables} onEdit={handleEdit} onDelete={handleDelete} onClone={handleClone} />
        }
      </section>
      ...
      <section>
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="flex gap-3">
            <button className="bg-blue-600 hover:bg-blue-700 text-white text-sm px-3 py-2 rounded shadow">Add to New Invoice</button>
            <button className="bg-gray-600 text-white text-sm px-3 py-2 rounded shadow hover:bg-gray-700">Export to Excel</button>
          </div>
        </div>
        <PayablesTable />
